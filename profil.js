window.PageProfil = (() => {
  ...
  function advancementFilterChipsHTML() {
    return window.ui.segmented(ADV_FILTERS, advFilter, "advfilter", "adv-filter-nav");
  }
  ...
  function advancementsGridHTML(advs, filter) {
    ...
    if (!list.length) {
      return `<div class="col-span-full">${window.ui.empty("Aucun succès dans cette catégorie.", "award")}</div>`;
    }
    return list
      .map((a) => {
        const dateLabel = a.unlocked && a.date
          ...
          : "Verrouillé";
        return `
        <div class="tile p-3 flex items-center gap-3 ${a.unlocked ? "" : "opacity-50"}" title="${esc(a.id)}">
          <span class="stat-icon ${a.unlocked ? "!text-ink !border-border2" : ""}">${window.icon(a.unlocked ? a.icon : "lock", 16)}</span>
          <div class="min-w-0 flex-1">
            <p class="text-[13px] font-medium truncate ${a.unlocked ? "text-ink" : "text-muted"}">${esc(a.title)}</p>
            <p class="text-[11.5px] text-dim">${dateLabel}</p>
          </div>
        </div>`;
      })
      .join("");
  ...
  function advancementsSectionHTML(advs) {
    if (!advs.length) {
      return window.ui.panel({
        title: "Succès",
        cls: "mb-4",
        body: window.ui.empty("Aucun succès enregistré pour ce joueur pour l'instant.", "award"),
      });
    }
    ...
    return window.ui.panel({
      title: "Succès",
      desc: `${window.fmt.int(unlockedCount)} sur ${window.fmt.int(advs.length)} débloqués (${pct} %)`,
      action: advancementFilterChipsHTML(),
      cls: "mb-4",
      body: `
        <div class="bar-track mb-4"><div class="bar-fill" style="width:${pct}%"></div></div>
        <div id="adv-grid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">${advancementsGridHTML(advs, advFilter)}</div>`,
    });
  }
